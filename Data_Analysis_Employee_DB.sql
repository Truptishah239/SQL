...
select
  department, sum(salary), avg(salary), var(salary)
from
   staff
group by
    department;
...
select
  department, sum(salary), avg(salary),var(salary), stdevp(salary)
from
   staff
group by
    department;
...
select
  department, sum(salary), avg(salary), round(var(salary),2), round(stdevp(salary),2)
from
   staff
group by
    department;
...
select
  job_title , department, CONCAT (job_title ,' - ', department) as title_dept
from
  staff;
...
select
   substring('abcdefghijkl', 5, 8) test_string;


/* Select Assistant job tiltles, and extract the main job category */
select
   job_title, substring(job_title, 10, len(job_title)) as asst_category
from
   staff
where
   job_title like 'Assistant%';
...
select job_title, replace (job_title,'Assistant', 'Assis.')
from staff
...
select
   replace('abcdefghijkl','cdef', 'CDEF' );



/* Change Assistant to Asst in job title */
select
   job_title, replace (job_title,'Assistant', 'Asst. ' )
from
   staff
where
   job_title like 'Assistant%';
...
select
   department, avg(salary), round(avg(salary),2), round(avg(salary),2)
from
   staff
group by
    department;
...
select
   department, avg(salary), round(avg(salary),2), ceiling(avg(salary))
from
   staff
group by
    department;
...
select
   department, avg(salary), round(avg(salary), 2), floor(avg(salary))
from
   staff
group by
    department;
...
select
   department, avg(salary), round(avg(salary), 3), round(avg(salary), 4)
from
   staff
group by
    department;
...
from
    staff
where
   job_title like '%Assistant%(II|IV)';
...
select
  ...
  s1.salary,
  s1.department,
(select (avg(salary)) from staff s2 where s1.department = s2.department) dept_avg
from staff s1;
...
select s.id, s.last_name, s.email, s.gender,
s.department, s.start_date,
s.salary,s.job_title,s.region_id,
cd.company_division, cr.company_regions
from staff s left join company_divisions cd
on s.department = cd.department left join
...
select
   company_division, company_regions, count(*) as Employee
from
   staff_div_reg
group by
...
select
   company_division, company_regions, count(*) as no_of_employee
from
   staff_div_reg
group by
   company_division, company_regions
...
select
   company_regions, country, count(*) as no_of_employee
